...
@section('content')
                    ...
                    <div class="card-body">
                        <table id="datatablesSimple">
                            ...
                            <tbody>
                                ...
                                @foreach ($penelitianBaru as $permohonan)
                                    <tr>
                                        ...
                                        <td>
                                            {{ \Carbon\Carbon::create($permohonan->tgl_awal)->translatedFormat('d F Y') . ' s/d ' . "\n " . \Carbon\Carbon::create($permohonan->tgl_akhir)->translatedFormat('d F Y') }}
                                        </td>
                                        <td>{{ $permohonan->keperluan }}</td>
                                        <td>{{ preg_replace('/[^a-zA-Z0-9 ,]/', '', $permohonan->lokasi_tujuan) }}</td>
                                        <td>
                    ...
                    <div class="card-body">
                        <table id="datatablesSimpleTwo">
                            ...
                            <tbody>
                                ...
                                @foreach ($penelitianBaru as $permohonan)
                                    <section id="permohonan{{ $permohonan->id_application }}">
                                        <tr>
                                            <td>{{ $permohonan->name }}</td>
                                                        ...
                                                        <tr class="text-center">
                                                            <td>
                                                                <button class="btn btn-sm btn-warning fa-solid fa-eye"
                                                                    ...
                                                                    onClick="window.open('{{ Route('kasi.lihat-penelitian', Crypt::encrypt($permohonan->id_application)) }}','_blank', 'location=yes,height=800,width=700,scrollbars=yes,status=yes');">
                                                                </button>
                                                            </td>
                                                            <td>
                                                                <a class="btn btn-primary btn-sm" data-bs-toggle="modal"
                                                                    data-bs-target="#staticBackdrop{{ $permohonan->id_application }}"
                                                                    title="Proses Permohonan"><i
    ...
    <!-- Agenda Modal -->
    @foreach ($penelitianBaru as $agenda)
        <div class="modal fade" id="staticBackdrop{{ $agenda->id_application }}" data-bs-backdrop="static"
            data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
            <div class="modal-dialog">
                <form action="{{ route('kasi.penelitian-baru-update', Crypt::encrypt($agenda->id_application)) }}"
                    method="post">
                    @csrf
                    @method('PUT')
                                            ...
                                            <div class="sbp-preview-content">
                                                <div class="mb-3">
                                                    ...
                                                    <select class="form-control form-select" id="status"
                                                        ...
                                                        <option value="tolak"
                                                            {{ old('status_surat') == 'tolak' ? 'selected' : '' }}>
                                                            Tolak
                                                        </option>
                                                        <option value="proses"
                                                            {{ old('status_surat') == 'proses' ? 'selected' : '' }}>
                                                            Setujui
                                                        </option>
                                                    </select>
                                                </div>
